...
public class HashMapPractice {
    public static void main(String[] args) {
        ...
        makeUpGrades.forEach((name, grade) -> {
            Integer firstTestGrade = originalGrades.get(name);
            Integer secondTestGrade = makeUpGrades.get(name);

            if (secondTestGrade > firstTestGrade) {
                originalGrades.put(name, grade);
            }
        });

        originalGrades.forEach((name, grade) -> System.out.println(name + " you ended the course with a " + grade + "."));
